...
import { getItemsPages, createItem, updateItem, deleteItem, type Item, type ItemsSelectionDTO } from '../api/items'
import Pagination from '../components/Pagination.vue'

const rows = ref<Item[]>([])
const page = reactive({ pageNumber: 1, pageSize: 10, total: 0 })
const filters = reactive<ItemsSelectionDTO>({ name: '', status: undefined })
const loading = ref(false)
const dialog = reactive({ visible:false, editingId: 0 })
const form = reactive<Item>({ title:'', description:'', status:0, type:'', contact:'', placeId: undefined })

async function fetchData(){
  loading.value = true
  try{
    const { data } = await getItemsPages({ filters, pageNumber: page.pageNumber, pageSize: page.pageSize })
    const r = data?.data || {}
...
function openCreate(){
  dialog.visible = true
  dialog.editingId = 0
  Object.assign(form, { title:'', description:'', status:0, type:'', contact:'', placeId: undefined })
}
...
async function save(){
  if(dialog.editingId){
    await updateItem({ form, id: dialog.editingId })
  }else{
...
<template>
  <div>
    <div class="header">
      ...
      <div class="actions">
        <button class="btn" @click="openCreate">新增物品</button>
    ...
    <div class="card grid cols-2" style="margin-bottom:14px">
      <div>
        <label>关键词</label>
        <input v-model="filters.name" class="input" placeholder="标题/类型/描述" @keyup.enter="() => { page.pageNumber=1; fetchData() }"/>
      </div>
      <div>
        <label>状态</label>
    ...
    <div v-if="dialog.visible" class="modal" style="position:fixed; inset:0; background:rgba(0,0,0,.45); display:grid; place-items:center">
      <div class="card" style="width:620px; max-width:95vw">
        <h3 style="margin-top:0">{{ dialog.editingId ? '编辑物品' : '新增物品' }}</h3>
        <div class="grid cols-2">
          ...
          <div>
            <label>状态</label>
            <select v-model="(form.status as any)" class="select">
              <option :value="0">未认领</option>
              <option :value="1">已认领</option>
            </select>
          </div>
